# Практическо упражнение – Generics и Traits II

## 🎯 Цели
- Да преговорим и задълбочим разбирането на generics, traits, асоциирани типове, trait bounds, trait objects и др.  
- Да отговорим на ключови въпроси и ситуации, свързани с темата на лекцията.  
- Да развием навик да аргументираме решенията си в Rust.

---

## ❓ Въпроси за самопроверка

1. **Каква е разликата между generics и асоцииран тип** в trait (напр. `fn foo<T>(...)` срещу `type Item;`) при дефиниране на trait?  

2. **Какво означава “trait bound”** и как се използва в дефиниция като:  
   ```rust
   fn convert_to_json<T: ToJson>(x: T) -> String
   ```  

3. **Защо trait с асоцииран тип често не може да бъде използван като trait object** (напр. `&dyn Iterator`) без допълнително ограничение? Дай пример.  

4. **Какво представлява `?Sized`** и кога е нужно да го използваме?  

5. Как дефиницията на  
   ```rust
   trait Aggregate {
       type Output;
       fn aggregate(data: &[Self]) -> Self::Output;
   }
   ```  
   и добавянето на `where Self: Sized` влияят върху това дали може да се използва trait object?  

6. **Какво е “dereference coercion”** (`Deref` и `DerefMut`) и кога компилаторът го прилага автоматично (пример: `&Box<T>` → `&T`)?  

7. **Защо операторите в Rust (като `+`, `*`) се дефинират чрез traits с асоциирани типове**, а не чрез traits с generic параметри за резултата?  

8. **Кога е по-подходящо да използваме trait object** (например `Box<dyn Trait>`) вместо generic тип параметър? Какви са компромисите между двата подхода?  

9. Напиши кратък псевдо-код на функция, която приема  
   ```rust
   &mut dyn Iterator<Item = i32>
   ```  
   и сумира всички елементи. Защо трябва да фиксираме `Item = i32`, за да може trait object да работи?  

10. Дадено е:  
    ```rust
    trait MyTrait {
        type Output;
        fn do_stuff(&self) -> Self::Output;
    }
    struct Foo;
    impl MyTrait for Foo {
        type Output = String;
        fn do_stuff(&self) -> String { "hello".to_string() }
    }
    ```
    Как бихме дефинирали функция, която работи с **някой тип**, който имплементира `MyTrait`, без да знаем конкретния `Output` тип? Какви са вариантите и ограниченията?

---

## 🧭 Инструкции
- Отдели време за всяка от въпросите — опитай се да формулираш отговор или да напишеш кратък код.  
- След това сравни отговора си с бележките от лекцията и/или допълнителна литература.  
- Запиши в отделен файл какво си научил или какво остава да изясниш.  
- На практика можеш да имплементираш няколко малки примера в Rust, изпробвайки generics + traits + асоциирани типове + trait objects.
